...
namespace BinaryTree
{
    ...
    internal class MyBinaryTree<T>
    {
        ...
        {
            if(_root != null) //루트가 존재한다면
            {
                _tmp1= _root;
                while (_tmp1 != null) //루트가 존재하지 않을 때까지 무한 순회
                {
                    // 내가 추가하려는 노드값이 현재 탐색 노드값보다 작은지
                    if (Comparer<T>.Default.Compare(item, _tmp1.Value) < 0)
                    {
                        if(_tmp1.Left != null)  //왼쪽 노드가 존재한다.
                        {
                            _tmp1 = _tmp1.Left;
                        }
                    ...
                    {
                        if(_tmp1.Right != null) //오른쪽 노드가 존재한다.
                        {
                            _tmp1 = _tmp1.Right;
                        }
                    ...
                    {
                        throw new InvalidOperationException($"[MybinaryTree] : {item} 은 이미 존재 합니다. 중복 허용하지 않음");
                    }
                }
            }
            else //루트가 존재하지 않는다면
        ...
        public Node<T> Find(T item)
        {
            ...
            while (_tmp1 != null) //루트가 존재하지 않을 때까지 무한 순회
            {
                ...
                if (Comparer<T>.Default.Compare(item, _tmp1.Value) < 0)
                {
                        _tmp1 = _tmp1.Left; //_tm1.Left가 null이라면 while에서 자동으로 나가짐
                }
                // 큰지?
                else if (Comparer<T>.Default.Compare(item, _tmp1.Value) > 0)
                {
        ...
        // 삭제 알고리즘
        // 삭제시 밸런싱 방법 :
        // 삭제한 노드의 오른쪽자식의 가장 왼쪽 으로 leaf를 탐색하고
        // 더이상 왼쪽이 없더라도 오른쪽이 있으면 또 오른쪽 자식으로가서 가장 왼쪽 leaf 를 탐색하는 것을 반복한 후
        ...
        public bool Remove(T item)
        {
            ...
            if(success)
            {
                ...
                if (_tmp1.Left == null && _tmp1.Right == null)
                {
                    ...
                    else if (dir > 0) //찾은 값이 오른자식이다.
                    {
                        _tmp2.Right = null;
                    }
                    else
                    {
                        throw new Exception($"[BinaryTree] : Wring child");
                    }
                    _tmp1 = null;
                }
                // 왼쪽 자식만 있을 경우
